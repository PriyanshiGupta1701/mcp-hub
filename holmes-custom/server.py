...
_LLM_NUM_RETRIES = int(os.environ.get("LITELLM_NUM_RETRIES", "3"))
litellm.num_retries = _LLM_NUM_RETRIES
litellm.retry_strategy = os.environ.get("LITELLM_RETRY_STRATEGY", "exponential_backoff_retry")

logging.info(
    f"Configured litellm.num_retries={_LLM_NUM_RETRIES} with exponential "
    f"backoff for transient errors (rate limits, timeouts, 5xx)"
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=os.environ.get("HOLMES_CORS_ORIGINS", "http://localhost:5173").split(","),
